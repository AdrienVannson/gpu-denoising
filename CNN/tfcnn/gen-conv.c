/*
  The parameters describing, for example, the size of the image or the number of features are
  given using hard-coded constants instead of push-constants. It allows to unroll more loops and
  gives far better performances. The code here is compiled many different times.
*/
...
{
    int OFFSET_WEIGHTS_SSBO; // Beginning of the weights
};
...
layout(std430, set = 1, binding = 0) readonly buffer BufWeights {
    float16_t weights[];
};
...
float16_t weight(const int i, const int j, const uint feature_in, const uint feature_out)
{
    ...
    return weights[OFFSET_WEIGHTS_SSBO + index];
}

float16_t bias(const uint i)
{
    return weights[OFFSET_WEIGHTS_SSBO + 9*NB_INPUT_FEATURES*NB_OUTPUT_FEATURES + i];
}

float16_t coef_of_image(const int i, const int j, const uint f_in)
{
    if (i < 0 || i >= GLOBAL_SIZE || j < 0 || j >= GLOBAL_SIZE) return float16_t(0.);
    ...
    if (f_in < NB_INPUT_FEATURES_1) {
        ...
        const uint pos = i/2 * GLOBAL_SIZE/2 + j/2;

        res = buff_in_1[INPUT_1_FEATURE_STRIDE * pos + f_in];
    } else {
        const uint pos = i * GLOBAL_SIZE + j;
    ...
    res = max(
        max(
            buff_in[INPUT_FEATURE_STRIDE * ((2*i) * 2*GLOBAL_SIZE + 2*j) + f_in],
            buff_in[INPUT_FEATURE_STRIDE * ((2*i) * 2*GLOBAL_SIZE + 2*j+1) + f_in]
        ),
        max(
            buff_in[INPUT_FEATURE_STRIDE * ((2*i+1) * 2*GLOBAL_SIZE + 2*j) + f_in],
            buff_in[INPUT_FEATURE_STRIDE * ((2*i+1) * 2*GLOBAL_SIZE + 2*j+1) + f_in]
        )
    );
#else
    res = buff_in[INPUT_FEATURE_STRIDE * (i * GLOBAL_SIZE + j) + f_in];
#endif
        ...
        [[unroll]] for (int j = 0; j < F_OUT_32 / 16; j++) {
            ...
            [[unroll]] for (int p = 0; p < 16 / 2; p++) {
                ...
                if (feature < NB_OUTPUT_FEATURES) {
                    ...
                    buff_out[OUTPUT_FEATURE_STRIDE * (line_img * GLOBAL_SIZE + column_img) + feature] = value;
                }
            }
        }
    }
